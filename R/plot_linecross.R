...
plot_linecross <- function(object, theta=-40, phi=20, margins=c(2,2,2,2), col.triangle="gray80", main = "default"){
  ...
  persp(x=AA, y=AA, reverse_mat, theta = theta, phi = phi, xlab="1-S",
        ylab="H",zlab="z", cex.lab=1, border="gray88", zlim=c(minimum, maximum), 
  ...
  mtext(bquote(R^2 == .(as.numeric(round(object$Rsquare,3))) ~ "AIC" == .(as.numeric(round(object$AIC,3)))), font=2, cex=1.2, side=1, adj=0.75, padj=0)
